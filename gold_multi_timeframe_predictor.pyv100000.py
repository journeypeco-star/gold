...
N_HORIZONS = 4  # reduced from 8 after Streamlit Cloud actively throttled this app's CPU - see comments below on the other cuts made alongside this one
PREDICTION_LOG_PATH = "prediction_log.json"
MAX_LOG_ENTRIES_PER_SERIES = 300  # per (timeframe, symbol, horizon) series; tracking covers all N_HORIZONS horizons, not just +1, so this was reduced from its original 500 to keep total file size reasonable
...
def _twelvedata_request(symbol, interval, outputsize=500, attempts=3, bar_seconds=None):
    ...
    for attempt in range(attempts):
        try:
            resp = requests.get(TD_BASE_URL, params={
                ...
                # Cache-busting: a unique value on every call, so nothing
                # sitting between this app and Twelve Data's live backend
                # (a CDN, a reverse proxy, response caching on an identical
                # repeated request) can serve back a stale copy of what
                # looks like "the same request." Twelve Data ignores
                # unrecognized parameters, so this is a no-op for them and
                # only affects whether something else short-circuits the
                # request with an old cached response.
                "_cb": str(time.time()),
            }, timeout=15)
            data = resp.json()
            ...
            # Self-healing staleness check: if the freshest bar returned is
            # already far older than this interval should ever be (allowing
            # generous slack for weekends/after-hours on non-24/7 markets),
            # treat it as a bad response and retry with a fresh cache-busted
            # request rather than silently accepting and displaying it -
            # added after live testing showed occasional stale responses
            # despite the underlying data being confirmed fresh on request.
            # Threshold matches the UI's own "abnormally stale" warning
            # level (previously mismatched - set at 6x here while the
            # warning banner showed at 5x, meaning the retry logic hadn't
            # even kicked in yet by the time the user was already seeing an
            # alarming banner about it).
            if bar_seconds:
                age_seconds = (pd.Timestamp.now(tz="UTC") - df.index[-1]).total_seconds()
                if age_seconds > bar_seconds * 3 and attempt < attempts - 1:
                    last_err = f"Received unexpectedly stale data ({age_seconds/60:.0f} min old); retrying"
...
@st.cache_data(ttl=1800, show_spinner=False)  # ~30 min: doubled again after Streamlit Cloud actively throttled this app's CPU - see sidebar note
def fetch_ohlc(symbol, interval, bar_seconds=None):
    return _twelvedata_request(symbol, interval, outputsize=500, bar_seconds=bar_seconds)
...
@st.cache_data(ttl=1800, show_spinner=False)  # matches fetch_ohlc's cadence so the whole pipeline refreshes together
def analyze_timeframe(tf_key, symbol, use_macro, asian_hours=(20, 22), london_hours=(2, 5), ny_hours=(7, 10)):
    ...
    raw = fetch_ohlc(symbol, tf["td_interval"], bar_seconds=tf["bar_seconds"])
    if raw.empty:
    ...
    for h in range(1, N_HORIZONS+1):
        ...
        if h == 1:
            # Only h=1 gets the full 3-model ensemble for its point
            # prediction - this is the headline number, worth the cost.
            ...
            # h>1: XGBoost-only for BOTH the point prediction and the
            # accuracy check - cut after Streamlit Cloud actively throttled
            # this app's CPU. This removes 2 of 3 model fits for every
            # horizon beyond the first, on top of the N_HORIZONS cut above.
            # horizon_agree has no real second/third model to compare
            # against here, so it's intentionally left unpopulated for
            # h>1 - the alert system below only checks agreement at h=1,
            # where it's still a genuine multi-model comparison.
            xgb_model = train_fast_xgb(X, y_h)
            xgb_p = float(predict_xgb(xgb_model, last_row)[0])
            combined = xgb_p
            ridge_p, rf_p = xgb_p, xgb_p  # only one real model at h>1 now; logged identically rather than leaving these undefined
...
with st.sidebar:
    ...
    st.caption(
        "New data is actually fetched from Twelve Data every ~30 minutes per "
        ...
        "288/day with substantial margin, while the page above can still "
    ...
    for tf in TIMEFRAMES:
        try:
            ...
            if "error" not in result:
                ...
                # by the end of the shown outlook." Summing all N_HORIZONS horizons
...
for tf in TIMEFRAMES:
    ...
    for h in range(1, N_HORIZONS+1):
        ...
        # h>1 no longer has a second/third model to compare against (cut
        # for CPU reasons - see the horizon loop above), so horizon_agree
        # only has an entry for h=1; this correctly and intentionally means
        # alerts only ever fire at h=1 now, where "3 models agree" is still
        # a genuine, real comparison rather than a single model against itself.
    ...
    with col:
        with st.container(border=True):
            ...
            try:
                ...
                # Shown in US Eastern alongside the UTC-based relative age so
                # this can be checked against a wall clock directly, with no
                # manual UTC conversion needed - tz_convert handles EDT/EST
                # automatically, avoiding the exact mixup that caused real
                # confusion when checking this by hand earlier.
                last_bar_et = result["last_bar_time"].tz_convert("America/New_York")
                st.caption(f"{freshness} Last bar: {bar_age_min:.0f} min ago ({last_bar_et.strftime('%-I:%M %p %Z')})")
