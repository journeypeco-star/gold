...
MAX_LOG_ENTRIES_PER_SERIES = 500  # caps file growth; oldest entries drop first
...
def check_past_predictions(log, tf_key, symbol, raw_df):
    ...
    score it - including which individual model was right, for the weight
    adaptation below."""
    n_checked = 0
    for entry in log:
    ...
    for h in range(1, N_HORIZONS+1):
        ...
        if h == 1:
            prediction_log = log_new_prediction(
                prediction_log, tf_key, symbol, 1, tf["bar_seconds"], combined,
                xgb_p, ridge_p, rf_p, current_close, feat_df.index[-1])
        ...
        if h == 1:
            ...
            xgb_only_acc_h, _, residuals_for_band = walk_forward_accuracy(X, y_h, train_fast_xgb, predict_xgb)
        ...
        try:
            if residuals_for_band is not None and len(residuals_for_band) >= 20:
                q10 = combined + float(np.percentile(residuals_for_band, 10))
                q90 = combined + float(np.percentile(residuals_for_band, 90))
                if q90 < q10:
                    q10, q90 = q90, q10
...
def build_ghost_candles(current_close, current_atr, horizon_preds, horizon_bands=None):
    """
    Builds the chained ghost-candle sequence. When a real empirical-residual
    band (p10/p90 of actual walk-forward prediction errors) is available for
    a horizon, the wick uses that genuine, data-estimated range instead of
    the ATR-multiplier heuristic - real historical error spread, not a
    guessed multiplier. Falls back to the heuristic per-horizon whenever the
